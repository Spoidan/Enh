...
import { createMutuellePayment, deleteMutuellePayment } from '@/lib/actions/mutuelle'
...
export function MutuelleClient({ payments }: { payments: Payment[] }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await createMutuellePayment(fd)
      if (res && 'error' in res) toast.error(res.error as string)
      else {
        toast.success('Paiement mutuelle enregistré')
        setOpen(false)
        setFileUrl('')
        setFileName('')
      }
  ...
  function handleDelete(id: string) {
    startTransition(async () => {
      const res = await deleteMutuellePayment(id)
      if (res && 'error' in res) toast.error(res.error as string)
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Nouveau paiement mutuelle</DialogTitle>
              </DialogHeader>
        ...
        <CardContent>
          {payments.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-12">
              Aucun paiement mutuelle enregistré.
            </p>
          ) : (
